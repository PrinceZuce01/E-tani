...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Butiran Pengguna berjaya disimpan.');
			...
			if($qry){
				$this->settings->set_flashdata('success','Butiran Pengguna berjaya dikemas kini.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
	...
	public function delete_users(){
		...
		if($qry){
			$this->settings->set_flashdata('success','Butiran Pengguna berjaya dipadamkan.');
	...
	public function save_vendor(){
		...
		if(isset($oldpassword) && !empty($id)){
			...
			if(md5($oldpassword) != $current_pass){
				$resp['status'] = 'failed';
				$resp['msg'] = ' Kata Laluan Semasa Salah';
				return json_encode($resp);
				exit;
			}
		}
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Nama pengguna sudah wujud";
		}else{
			if(empty($id)){
			...
			if($save){
				...
				if(empty($id)){
					if(strpos($_SERVER['HTTP_REFERER'], 'vendor/register.php') > -1){
						$resp['msg'] = " Akaun anda telah berjaya didaftarkan.";
					}else{
						$resp['msg'] = " Akaun Petani telah berjaya didaftarkan.";
					}
				}else{
					if($this->settings->userdata('login_type') == 2){
						$resp['msg'] = " Butiran akaun anda telah berjaya dikemas kini.";
					}else{
						$resp['msg'] = " Butiran Akaun Petani telah berjaya dikemas kini.";
					}	
				}
				...
				$resp['status'] = 'failed';
				$resp['msg'] = " Ralat berlaku semasa menyimpan butiran akaun.";
	...
	public function delete_vendor(){
		...
		if($qry){
			$this->settings->set_flashdata('success',' Butiran Petani berjaya dipadamkan.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = 'Ralat berlaku semasa memadam data.';
			$resp['error'] = $this->conn->error;
		}
		return json_encode($resp);
	}
			...
			if($save){
				...
				if(empty($id)){
					if(strpos($_SERVER['HTTP_REFERER'], 'client/register.php') > -1){
						...
						$resp['msg'] = " Akaun Pembeli telah berjaya didaftarkan.";
					}
				}else{
					if($this->settings->userdata('login_type') == 3){
						$resp['msg'] = " Butiran akaun anda telah berjaya dikemas kini.";
					}else{
						$resp['msg'] = " Butiran Akaun Pembeli telah berjaya dikemas kini.";
					}	
				}
	...
	public function delete_client(){
		...
		if($qry){
			$this->settings->set_flashdata('success',' Butiran Pembeli berjaya dipadamkan.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = 'Ralat berlaku semasa memadam data.';
