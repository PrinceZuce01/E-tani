...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT o.*,v.shop_name,v.contact as vcontact, v.code as vcode from `order_list` o inner join vendor_list v on o.vendor_id = v.id where o.id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	<div class="row">
        <div class="col-3 border bg-gradient-primary"><span class="">Kod Rujukan</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($code) ? $code : '' ?></span></div>
        <div class="col-3 border bg-gradient-primary"><span class="">Petani</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($shop_name) ? $vcode.' - '.$shop_name : '' ?></span></div>
        <div class="col-3 border bg-gradient-primary"><span class="">No Tel Petani</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($shop_name) ? $vcontact : '' ?></span></div>
        <div class="col-3 border bg-gradient-primary"><span class="">Alamat Bungkusan</span></div>
	...
	<div class="text-right">
        <?php if(isset($status) && $status == 0): ?>
		    <button class="btn btn-default bg-gradient-danger text-light btn-sm btn-flat" type="button" id="cancel_order">Batal Tempahan</button>
        <?php endif; ?>
...
<script>
    $(function(){
        $('#cancel_order').click(function(){
            _conf("Adakah anda pasti akan membatalkan tempahan ini?","cancel_order",['<?= isset($id) ? $id : '' ?>'])
        })
    })
