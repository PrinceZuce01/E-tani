...
$(document).ready(function() {
                ...
                success: function(resp) {
                    if (resp) {
                        ...
                        if (resp.status == 'success') {
                            ...
                            var _frm = $('#login-frm')
                            var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Kata Nama dan Kata Kunci tidak tepat</div>"
                            _frm.prepend(_msg)
    ...
    $('#system-frm').submit(function(e) {
        ...
        $.ajax({
            ...
            success: function(resp) {
                if (resp == 1) {
                    // alert_toast("Data Berjaya Disimpan",'success')
                    location.reload()
                } else {
                    $('#msg').html('<div class="alert alert-danger err_msg">Terdapat masalah teknikal</div>')
                    end_load()
                }
            }
        })
